 ...
 * feature carries @module/@be|@fe/@db|@api and a domain sub-tag (@pricing,
 * @booking, @availability, @security, @frankfurter), and a Scenario Outline carries one
...
const TIER = (tags) => {
  ...
  if (tags.has('@frankfurter') && tags.has('@api')) return 'frankfurter-api';
  if (tags.has('@stay') && tags.has('@security')) return 'mini-stay-security';
  if (tags.has('@stay') && tags.has('@pricing')) return 'mini-stay-pricing';
  if (tags.has('@stay') && tags.has('@booking')) return 'mini-stay-booking';
  if (tags.has('@stay') && tags.has('@availability')) return 'mini-stay-availability';
  if (tags.has('@stay') && tags.has('@db')) return 'mini-stay-db';
  if (tags.has('@stay') && tags.has('@fe')) return 'mini-stay-fe';
  return 'unknown';
};
...
function main() {
  ...
  const fixture = {
    meta: {
      domain: 'booking',
      targets: {
        'mini-stay-db': 'mini-stay SQLite, written by the service (be/db, both stacks open the same file)',
        'mini-stay-pricing': 'mini-stay pricing -- nights x nightly rate plus integer-bps tax, and cancellation refund tiers (be/api)',
        'mini-stay-booking': 'mini-stay booking lifecycle -- hold, confirm, check-in, check-out, cancel, hold expiry, no-show (be/api)',
        'mini-stay-availability': 'mini-stay inventory -- no overbooking a room for any night, including under concurrent requests (be/api)',
        'mini-stay-security': 'mini-stay authorization boundaries -- no/wrong/forged token, role confusion, cross-guest access (be/api)',
        'mini-stay-fe': 'mini-stay HTML pages -- hotels, rooms, a booking, a guest\'s bookings and admin overview (Playwright)',
        'frankfurter-api': 'frankfurter.dev public FX API (no key) -- historical-rate equality and latest shape, used to convert a stay price',
      },
      idPolicy: 'immutable',
      nextId,
  ...
  const expected = {
    why: [
      ...
      'mini-stay cases (DB, pricing, booking, availability, security, FE) are deterministic against the local service and its SQLite; they grade Blocked only when the service is not running.',
      'Frankfurter cases grade Blocked when the live API or the network is unavailable -- an outage is never a rate being wrong.',
    ...
    summary: {
      ...
      Failed: 0,
      Blocked: 0,
      note: 'nominal: mini-stay running and seeded, Frankfurter reachable, a browser available for the FE tier',
    },
...
function renderMd(rows) {
  const out = ['# Hotel booking — test cases', '', `${rows.length} cases across a self-written mini-stay service (a real SQLite store behind the search, hold, booking lifecycle and cancellation flows) and the live Frankfurter FX API. Generated from \`../features/*.feature\` by \`build.js\`; do not edit by hand.`, ''];
