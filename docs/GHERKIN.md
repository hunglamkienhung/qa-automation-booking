...
- `@module:NN-name`, and a tier tag: `@be`/`@fe` with `@db`/`@api`, plus a
  target tag (`@stay`, `@frankfurter`) and a domain sub-tag (`@pricing`,
  `@booking`, `@availability`, `@security`). The two stacks select the same set —
...
```gherkin
But "tomorrow's exact FX rate" cannot be verified because
    "Frankfurter's latest rate moves with the market, not a settled value"
```
