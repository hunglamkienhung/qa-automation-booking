...
pytest_plugins = [
    "be.db.steps.stay_steps",
    "be.api.steps.pricing_steps",
    "be.api.steps.booking_steps",
    "be.api.steps.availability_steps",
    "be.api.steps.security_steps",
    "be.api.steps.frankfurter_steps",
]
...
def bind_case(request, qa):
    ...
    if case_id is None:
        return
    qa.case(case_id)
    for slot in ("store", "api", "quote", "booking"):
